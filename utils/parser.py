...
def parse_session(json_data: dict) -> dict:
    ...
    cp_dur_h = ep1_start_h  # CP runs from 0 to EP1 start

    # Calculate peak as time of maximum smoothed height from measurements.
    # peakElapsedSeconds from heightAnalysis is used only as fallback when
    # height data is missing (e.g. incomplete session).
    if not meas["height"].isna().all():
        h_smooth = meas["height"].rolling(10, center=True, min_periods=1).median()
        peak_idx = h_smooth.idxmax()
        peak_h = float(meas.loc[peak_idx, "elapsedSeconds"]) / 3600
    else:
        peak_sec = ha.get("peakElapsedSeconds")
        peak_h = peak_sec / 3600 if peak_sec is not None else None
